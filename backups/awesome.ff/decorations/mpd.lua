...
volume_bar:connect_signal("button::press", function(_, lx, __, button)
    if button == 1 then
        awful.spawn.with_shell("playerctl -p ncspot volume "..tostring(math.ceil(lx * 100 / volume_bar.forced_width)))
    end
end)
...
volume:buttons(gears.table.join(
    ...
    awful.button({ }, 4, function ()
        awful.spawn.with_shell("playerctl -p ncspot volume +5")
    end),
...
local mpd_create_decoration = function (c)
    ...
    -- Toolbar
    -- The functions that send keys to the ncplayerctl -p ncspotpp client assume that
    -- you are using default ncplayerctl -p ncspotpp keybindings. Otherwise, you will
        ...
        {
            {
                ...
                control_button(c, "", x.color5, dpi(30),
                    ...
                    function()
                        awful.spawn.with_shell("playerctl -p ncspot toggleoutput mpd_fifo")
                    end
                    ),
                random_button,
                loop_button,
