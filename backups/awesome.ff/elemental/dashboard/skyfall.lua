...
local function create_bookmark(name, path)
    ...
    bookmark:buttons(gears.table.join(
        ...
        awful.button({ }, 3, function ()
            awful.spawn.with_shell(user.terminal.." -e "..path.. " ls")
            dashboard_hide()
        end)
    ))
...
local bookmarks = wibox.widget {
    ...
    create_bookmark("DOWNLOADS", "/home/marhearn/Downloads"),
    create_bookmark("TRUENAS", "/mnt/nfs/truenas"),
    create_bookmark("PICS", "/mnt/nfs/truenas/media/pictures/psus/fixed"),
    create_bookmark("SCRIPTS", "/home/marhearn/.scripts"),
    spacing = dpi(10),
    layout = wibox.layout.fixed.vertical
}
...
local urls = wibox.widget {
    ...
    create_url("Truenas", "truenas.lo"),
    create_url("CloudMarsh", "cloudmarsh.com"),
    spacing = dpi(10),
    layout = wibox.layout.fixed.vertical
}
