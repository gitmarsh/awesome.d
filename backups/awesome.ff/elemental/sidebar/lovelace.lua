...
local binclock = require("elemental.binclock"){
    height = dpi(62),
    show_seconds = false,
    color_active = x.color2,
    color_inactive = x.color8
}
...
local disknet_space = require("noodle.disknet")
disknet_space.font = "sans 14"
local disknet_icon = wibox.widget.imagebox(icons.image.files)
disknet_icon.resize = true
disknet_icon.forced_width = icon_size
disknet_icon.forced_height = icon_size
...
disk:buttons(gears.table.join(
    awful.button({ }, 1, function ()
        awful.spawn("dolphin /home/marhearn", {floating = true})
    end),
    awful.button({ }, 3, function ()
        awful.spawn(user.file_manager .. " /data", {floating = true})
    end)
))
disknet:buttons(gears.table.join(
    awful.button({ }, 1, function ()
        awful.spawn("dolphin /mnt/nfs/truenas", {floating = true})
    end),
    awful.button({ }, 3, function ()
        awful.spawn(user.file_manager .. " /mnt/nfs/truenas", {floating = true})
    end)
))
...
sidebar:setup {
    { ----------- TOP GROUP -----------
        ...
        wibox.container.margin(binclock.widget, dpi(110), dpi(40)),
        date,
